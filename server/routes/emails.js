...
router.post('/send', async (req, res) => {
  try {
    ...
    try {
      ...
      signature = await signMessage(messageToSign, privateKey);
...
router.post('/verify', async (req, res) => {
  try {
    ...
    if (missingFields.length > 0) {
      console.log('❌ Missing fields for verification:', missingFields);
      return res.status(400).json({ 
    ...
    try {
      ...
      console.log('Signature to verify:', signature.substring(0, 50) + '...');
      console.log('Public key to use:', senderPublicKey.substring(0, 100) + '...');
      ...
      res.json({ 
        isValid, 
        verified: isValid,
        message: isValid ? 'Signature is valid' : 'Signature verification failed - signature does not match',
