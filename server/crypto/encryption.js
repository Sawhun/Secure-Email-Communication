...
export function signMessage(message, privateKeyPem) {
  try {
    ...
    console.log('✅ Hash created for message');
...
export function verifySignature(message, signature, publicKeyPem) {
  try {
    ...
    console.log('Message content:', message.substring(0, 200) + (message.length > 200 ? '...' : ''));
    console.log('Signature length:', signature.length);
    console.log('Signature (first 100 chars):', signature.substring(0, 100));
    console.log('Public key format check:', publicKeyPem.substring(0, 100) + '...');
    ...
    if (!message || !signature || !publicKeyPem) {
      ...
      console.log('Has publicKey:', !!publicKeyPem);
      return false;
    }

    if (!publicKeyPem.includes('-----BEGIN PUBLIC KEY-----')) {
      console.log('❌ Invalid public key format for verification');
      console.log('Public key preview:', publicKeyPem.substring(0, 200));
      return false;
    }
    ...
    try {
      ...
      console.error('Public key that failed:', publicKeyPem.substring(0, 200));
      return false;
    }
    ...
    const md = forge.md.sha256.create();
    md.update(message, 'utf8');
    console.log('✅ Hash created for verification');
    
    // Decode signature
    let decodedSignature;
    try {
      decodedSignature = forge.util.decode64(signature);
      console.log('✅ Signature decoded, length:', decodedSignature.length);
    } catch (decodeError) {
      console.error('❌ Invalid signature format:', decodeError);
      console.error('Signature that failed to decode:', signature.substring(0, 100));
      return false;
    }
    ...
    try {
      ...
      if (!isValid) {
        console.log('❌ Signature verification failed - signature does not match message');
        console.log('Message hash:', forge.util.encode64(md.digest().bytes()));
      }
    } catch (verifyError) {
